...
const demoTarget = (mode: GameMode, index: number): Target => {
  const now = Date.now()
  const motion = mode === 'zombie' ? { spawnedAt: now, expiresAt: now + 8500 + index * 650, kind: index % 5 === 3 ? 'armored' as const : index % 5 === 4 ? 'exploder' as const : 'normal' as const } : mode === 'balloon' ? { spawnedAt: now, expiresAt: now + 7000 + index * 500, kind: index === 4 ? 'giant' as const : index === 3 ? 'bomb' as const : index > 0 ? 'chain' as const : 'balloon' as const } : mode === 'racing' ? { kind: index === 4 ? 'nitro' as const : index > 1 ? 'corner' as const : 'speed' as const } : mode === 'treasure' ? { kind: index === 4 ? 'vault' as const : index === 3 ? 'trap' as const : index === 2 ? 'map' as const : index === 1 ? 'key' as const : 'chest' as const } : mode === 'crown' ? { kind: index === 0 ? 'crown' as const : 'guard' as const } : {}
  const word = WORDS[Math.floor(Math.random() * WORDS.length)] ?? WORDS[0]
  return { id: `demo-${index}-${now}-${Math.random()}`, text: (mode === 'racing' && motion.kind === 'nitro') || (mode === 'treasure' && motion.kind === 'vault') ? `${word} ${WORDS[(index + 6) % WORDS.length]}` : word, points: 100 + index * 20, ...motion }
}
...
function App() {
  ...
  const onServerMessage = useCallback((message: ServerMessage) => {
    if (message.type === 'connected') {
      ...
      if (message.playerId === playerId) {
        setToast(message.outcome === 'success' ? { text: `${gameMode === 'zombie' ? 'ZAP' : gameMode === 'shoot' ? 'BANG' : gameMode === 'balloon' ? message.scoreDelta < 0 ? 'BOMB' : 'POP' : gameMode === 'crown' ? 'CROWN' : 'CATCH'}! ${message.scoreDelta >= 0 ? '+' : ''}${message.scoreDelta}`, tone: message.scoreDelta < 0 ? 'bad' : 'good' } : message.outcome === 'claimed' ? { text: '한발 늦었어요!', tone: 'info' } : { text: 'MISS!', tone: 'bad' })
  ...
  const submit = (event: FormEvent) => {
    ...
    setToast({ text: target.kind === 'bomb' ? 'BOMB! -100' : target.kind === 'trap' ? 'TRAP! -80' : `${gameMode === 'zombie' ? 'ZAP!' : gameMode === 'balloon' ? 'POP!' : gameMode === 'treasure' ? 'LOOT!' : gameMode === 'crown' ? 'CROWN!' : 'CATCH!'} +${scoreDelta}`, tone: target.kind === 'bomb' || target.kind === 'trap' ? 'bad' : 'good' }); triggerFeedback(target.kind === 'bomb' || target.kind === 'trap' ? 'miss' : 'success', target.id); setInput('')
  ...
  return <div className={`app ${reduced ? 'reduced' : ''} ${keyboardOpen ? 'keyboard-open' : ''}`}>
    ...
    {screen === 'game' && <main className={`game mode-${gameMode} feedback-${inputFeedback ?? 'idle'} ${isSpectator ? 'spectating' : ''}`} onClick={() => { if (!isSpectator) inputRef.current?.focus() }}>
      ...
      <section className={`arena ${gameMode === 'shoot' ? 'shooting-arena' : gameMode === 'zombie' ? 'zombie-arena' : gameMode === 'balloon' ? 'balloon-arena' : gameMode === 'racing' ? 'racing-arena' : gameMode === 'treasure' ? 'treasure-arena' : gameMode === 'crown' ? 'crown-arena' : ''}`}>
        ...
        {gameMode === 'treasure' && <div className="treasure-inventory"><span>🔑 KEY <b>{state.modeState?.treasure?.[playerId]?.keys ?? 0}</b></span><span>🗺 MAP <b>{state.modeState?.treasure?.[playerId]?.maps ?? 0}/3</b></span><em>지도 3장을 모으면 +200</em></div>}
        {gameMode === 'crown' && <div className={`crown-status ${state.modeState?.crown?.holderId === playerId ? 'mine' : ''}`}><div className="crown-emblem">♛</div><div><small>CURRENT RULER</small><strong>{state.players.find(player => player.id === state.modeState?.crown?.holderId)?.nickname ?? '왕관의 주인이 없습니다'}</strong><span>{state.modeState?.crown?.holderId ? `${((state.modeState.crown.heldMs[state.modeState.crown.holderId] ?? 0) / 1000).toFixed(1)}초 방어 중` : '황금 타깃을 먼저 입력하세요'}</span></div><b>×{state.modeState?.crown?.streak ?? 0}<small>POWER</small></b></div>}
        <div className={`targets ${gameMode === 'shoot' ? 'shooting-targets' : gameMode === 'zombie' ? 'zombie-targets' : gameMode === 'balloon' ? 'balloon-targets' : gameMode === 'racing' ? 'racing-targets' : gameMode === 'treasure' ? 'treasure-targets' : gameMode === 'crown' ? 'crown-targets' : ''}`}>{state.targets.map((target, i) => {
