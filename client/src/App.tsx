...
function App() {
  ...
  const treasureMapProgress = myTreasure.maps % 3
  ...
  return <div className={`app ${reduced ? 'reduced' : ''} ${keyboardOpen ? 'keyboard-open' : ''}`}>
    ...
    {screen === 'game' && <main className={`game mode-${gameMode} feedback-${inputFeedback ?? 'idle'} ${isSpectator ? 'spectating' : ''}`} onClick={() => { if (!isSpectator) inputRef.current?.focus() }}>
      ...
      <section className={`arena ${gameMode === 'shoot' ? 'shooting-arena' : gameMode === 'zombie' ? 'zombie-arena' : gameMode === 'balloon' ? 'balloon-arena' : gameMode === 'racing' ? 'racing-arena' : gameMode === 'treasure' ? 'treasure-arena' : gameMode === 'crown' ? 'crown-arena' : ''}`}>
        ...
        {gameMode === 'treasure' && <><div className={`treasure-inventory ${myTreasure.keys > 0 ? 'has-key' : ''} ${treasureMapProgress === 2 ? 'map-near' : ''}`}><span>🔑 KEY <b>{myTreasure.keys}</b></span><span>🗺 MAP <b>{treasureMapProgress}/3</b></span><em>{myTreasure.keys > 0 ? '금고 개방 가능 · +400점!' : treasureMapProgress === 2 ? '지도 한 장이면 세트 완성 · +200점!' : '열쇠→금고 또는 지도 3장을 노리세요'}</em></div><div className="treasure-plan"><span className={myTreasure.keys > 0 ? 'ready' : ''}>① 열쇠 획득</span><b>→</b><span className={myTreasure.keys > 0 ? 'ready jackpot' : ''}>② 금고 +400</span><i>또는</i><span className={treasureMapProgress === 2 ? 'ready' : ''}>지도 {treasureMapProgress}/3 · 완성 +200</span><i>·</i><span className="risk">? 상자는 함정 가능</span></div></>}
        ...
        {gameMode === 'crown' && <>
          <div className={`crown-throne ${isCrownHolder ? 'mine' : crownHolderId ? 'occupied' : 'vacant'}`} aria-hidden="true"><i>♛</i><span>{crownHolder ? `${crownHolder.nickname}의 왕좌` : '비어 있는 왕좌'}</span><b>{crownHolder ? `${crownHeldSeconds.toFixed(1)}s REIGN` : 'CLAIM NOW'}</b></div>
          <div key={crownHolderId ?? 'unclaimed'} className={`crown-status crown-arrival ${isCrownHolder ? 'mine' : crownHolderId ? 'rival' : 'unclaimed'}`}><div className="crown-emblem">♛</div><div><small>{isCrownHolder ? 'YOU ARE THE RULER' : crownHolder ? 'STEAL THE CROWN' : 'CROWN UNCLAIMED'}</small><strong>{isCrownHolder ? '왕관을 지키세요!' : crownHolder ? `${crownHolder.nickname}에게서 빼앗으세요!` : '왕관을 먼저 차지하세요!'}</strong><span>{isCrownHolder ? `현재 초당 +${crownRate}점 · 방어할수록 상승` : crownHolder ? '황금 왕관 단어를 먼저 입력하면 즉시 탈취' : '황금 왕관 단어를 가장 먼저 입력하세요'}</span></div><b>×{crownPower}<small>POWER</small></b></div>
          <div className="crown-power"><small>ROYAL POWER</small><span>{Array.from({ length: 5 }, (_, level) => <i key={level} className={level < crownPower ? 'charged' : ''} />)}</span><b>{isCrownHolder ? `+${crownRate}/SEC` : crownHolder ? 'TAKEOVER +150' : 'FIRST CLAIM +150'}</b></div>
          <div className="crown-players">{state.players.map(player => <div key={player.id} className={`${player.id === playerId ? 'mine' : ''} ${player.id === crownHolderId ? 'ruler' : ''}`}><i>{player.id === crownHolderId ? '♛' : '·'}</i><span>{player.nickname}</span><small>{((state.modeState?.crown?.heldMs[player.id] ?? 0) / 1000).toFixed(1)}s</small></div>)}</div>
          <div className={`crown-role-banner ${isCrownHolder ? 'defend' : 'steal'}`}><b>{isSpectator ? '전체 상황 관전 중' : isCrownHolder ? '◆ 방어 단어 3개 중 하나를 입력해 왕관을 강화하세요' : '♛ 황금 왕관 단어 하나를 정확히 입력해 즉시 탈취하세요'}</b><span>{isCrownHolder ? 'DEFEND' : 'CAPTURE'}</span></div>
        </>}
