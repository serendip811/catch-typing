...
const demoTarget = (mode: GameMode, index: number): Target => {
  const now = Date.now()
  const motion = mode === 'zombie' ? { spawnedAt: now, expiresAt: now + 8500 + index * 650, kind: index % 5 === 3 ? 'armored' as const : index % 5 === 4 ? 'exploder' as const : 'normal' as const } : mode === 'balloon' ? { spawnedAt: now, expiresAt: now + 7000 + index * 500, kind: index === 4 ? 'giant' as const : index === 3 ? 'bomb' as const : index > 0 ? 'chain' as const : 'balloon' as const } : mode === 'racing' ? { kind: index === 4 ? 'nitro' as const : index > 1 ? 'corner' as const : 'speed' as const } : mode === 'treasure' ? { kind: index === 4 ? 'vault' as const : index === 3 ? 'trap' as const : index === 2 ? 'map' as const : index === 1 ? 'key' as const : 'chest' as const } : {}
...
function App() {
  ...
  const startGame = () => {
    ...
    const now = Date.now(); setSeconds(60); setState(s => ({ ...s, phase: 'playing', targets: starterTargets(gameMode), modeState: gameMode === 'zombie' ? { baseHealth: 100, wave: 1, teamKills: 0 } : gameMode === 'racing' ? { trackLength: 100, race: Object.fromEntries(s.players.map(player => [player.id, { distance: 0, nitro: 0 }])) } : gameMode === 'treasure' ? { treasure: Object.fromEntries(s.players.map(player => [player.id, { keys: 0, maps: 0 }])) } : undefined, endsAt: now + 60000, durationMs: 60000 })); setScreen('game'); window.setTimeout(() => inputRef.current?.focus(), 50)
  }
  ...
  const submit = (event: FormEvent) => {
    ...
    setToast({ text: target.kind === 'bomb' ? 'BOMB! -100' : target.kind === 'trap' ? 'TRAP! -80' : `${gameMode === 'zombie' ? 'ZAP!' : gameMode === 'balloon' ? 'POP!' : gameMode === 'treasure' ? 'LOOT!' : 'CATCH!'} +${scoreDelta}`, tone: target.kind === 'bomb' || target.kind === 'trap' ? 'bad' : 'good' }); triggerFeedback(target.kind === 'bomb' || target.kind === 'trap' ? 'miss' : 'success', target.id); setInput('')
  ...
  return <div className={`app ${reduced ? 'reduced' : ''} ${keyboardOpen ? 'keyboard-open' : ''}`}>
    ...
    {screen === 'game' && <main className={`game mode-${gameMode} feedback-${inputFeedback ?? 'idle'} ${isSpectator ? 'spectating' : ''}`} onClick={() => { if (!isSpectator) inputRef.current?.focus() }}>
      ...
      <section className={`arena ${gameMode === 'shoot' ? 'shooting-arena' : gameMode === 'zombie' ? 'zombie-arena' : gameMode === 'balloon' ? 'balloon-arena' : gameMode === 'racing' ? 'racing-arena' : gameMode === 'treasure' ? 'treasure-arena' : ''}`}>
        <p className="arena-label">{gameMode === 'shoot' ? 'TRACK · TYPE · SHOOT!' : gameMode === 'zombie' ? 'DEFEND THE LAST KEYBOARD!' : gameMode === 'balloon' ? 'TYPE · POP · CHAIN!' : gameMode === 'racing' ? 'TYPE · BOOST · OVERTAKE!' : gameMode === 'treasure' ? 'CHOOSE · TYPE · DISCOVER!' : 'TYPE ONE & PRESS ENTER'}</p>
        ...
        <div className={`targets ${gameMode === 'shoot' ? 'shooting-targets' : gameMode === 'zombie' ? 'zombie-targets' : gameMode === 'balloon' ? 'balloon-targets' : gameMode === 'racing' ? 'racing-targets' : gameMode === 'treasure' ? 'treasure-targets' : ''}`}>{state.targets.map((target, i) => {
